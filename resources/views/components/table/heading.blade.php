...
<th scope="col"
        {{ $attributes->merge(['class' => 'px-3 py-2.5 text-left text-sm font-bold'])->only('class') }}
>
    @unless ($sortable)
        <span class="flex">{{ $slot }}</span>
    @else
        <button {{ $attributes->except('class') }} class="flex items-center space-x-1 text-left text-sm font-bold group focus:outline-none focus:underline">
            <span>{{ $slot }}</span>
