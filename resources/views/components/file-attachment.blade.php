        ...
        >
            ...
            <div wire:loading.flex wire:target="{{ $attributes->wire('model')->value }}" wire:loading.class="w-full">
                @if ($mode === 'profile' && $file)
                    <div class="select-none text-sm text-gray-500 flex flex-1 items-center justify-center text-center p-4 flex-1">
            ...
            <div class="flex items-center justify-center flex-1 px-4 py-2" wire:loading.class="hidden" wire:target="{{ $attributes->wire('model')->value }}">
                @if($slot->isEmpty())
                    @if($multiple)
                        <x-icon.document class="text-gray-400 -rotate-2" size="8"/>
                    @else
                        @if ($mode === 'profile')
                    ...
                    @if ($mode === 'attachment')
                        <span class="ml-2 text-gray-600">{{ is_array($file) ? 'Browse files' : 'Browse file' }} | <span class="text-sm">JSON</span></span>
                    @endif
                @else
                    {{ $slot }}
                @endif
                ...
                @foreach($file as $key => $f)
                    <div class="py-3 flex {{ !$loop->last ? 'border-b border-gray-200' : '' }}">
                        <div class="w-16 mr-4 flex-shrink-0 shadow-xs rounded-lg" >
                            ...
                            @else
                                <div class="w-16 h-16 bg-gray-100 text-gray-400 flex items-center justify-center rounded-lg border border-gray-100">
                                    <x-icon.document/>
                                </div>
                            @endif
                        </div>
                        <div>
                            ...
                            <div class="flex items-center space-x-1">
                                <div class="text-xs text-gray-500">{{ $this->humanFileSize($f->getSize())}}</div>
